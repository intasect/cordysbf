...
/**
 * Database provider class for SQLServer.
 *
 * @author mpoyhone
 */
public class SqlServerDatabaseProvider extends DatabaseProviderBase
{
    /**
    ...
    {
        if (dbcfg.getServerName() == null || dbcfg.getDbname() == null) {
            throw new BuildException("Either database URL or server and database name must be set.");
        }
        
        return String.format("jdbc:odbc:Driver={SQL Server};Server=%s;Database=%s", 
                dbcfg.getServerName(), dbcfg.getDbname());
    }
}
